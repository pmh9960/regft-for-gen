...
def torch_load(save_path, device=None) -> torch.nn.Module:
...
def get_model_weights(model):
    return torch.cat([p.data.cpu().ravel() for p in list(model.parameters())])


def update_model_weights(model, p):
    offset = 0
    for parameter in model.parameters():
        size = int(torch.prod(torch.tensor(parameter.size())))
